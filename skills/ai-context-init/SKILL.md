...
3. Check Trellis the same way. If installed but the project is uninitialized, initialize project-level Trellis with the installed version's supported command and verify it is usable. Preserve existing valid state; repair or refresh only when stale/broken.
   - Trellis platform glue is per-platform. A project initialized for one platform (for example `.codex/` only) still lacks the hooks/commands/agents other platforms need; when the current platform's surfaces are missing, run the installed CLI's init for that platform with skip-existing semantics (e.g. `trellis init --claude --skip-existing --yes`) so existing task state, `.trellis/`, and other platforms' files are preserved. Verify the platform glue afterwards (e.g. `trellis platforms` lists the current platform; the per-turn workflow-state hook returns usable output).
4. When Trellis is available, treat it as the project-local task/state workflow for the current development work. Global `project-delivery-flow` remains the cross-project orchestration entry and should map its phases into the existing Trellis state instead of creating a competing second plan. On machines where wxh-dev-standard is installed, `python3 ~/.wxh-dev-standard/scripts/sync-claude-hooks.py` installs the user-global Claude Code commit gate (blocks `git commit` in Trellis projects without an active task; escape keyword `no-trellis`); run it once when the gate is missing from `~/.claude/settings.json`.
